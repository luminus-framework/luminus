...
CSRF protection provided by the [ring-anti-forgery](https://github.com/ring-clojure/ring-anti-forgery) middleware is enabled by default. The `+swagger` profile disables CSRF for the `/api/*` routes as they're meant to be accessible by remote clients that aren't coming from a page generated by the server.

The middleware configuration is found in the `<app>.middleware` namespace of our application. The custom middleware that enables filtering CSRF by the request URI is seen below:
